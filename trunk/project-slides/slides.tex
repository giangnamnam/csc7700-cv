 ...
 \begin{itemize}
   ...
   \begin{itemize}
    \item Stop-sign detector, using the notion of integral
          images from SURF
    \item Warning sign detector, using a perspective transformation
          to transform the image to be parallel to the viewing plane, then SURF 
   \end{itemize} 
 \end{itemize} 
}
...
\frame{ \frametitle{Warning Signs}
  \begin{itemize}
    ...
    \item In particular, we assume that we know the perspective
          ...
          a perspective transform to the input image to make the sign
          upright and parallel to the viewing plane, apply
          SURF to the image using each model, and match.
  \end{itemize} 
}

\frame{ \frametitle{} 
\begin{center}
\includegraphics[width=.7\textwidth]{pos-warning.jpg} 
\end{center} }

\frame{ \frametitle{Warning Signs}
  \begin{itemize}
    \item We first hand-annotated $N$=74 images using a MATLAB code.
...
\begin{equation}
\end{equation}
    \item We give an example on the following slide.
  \end{itemize} 
}
