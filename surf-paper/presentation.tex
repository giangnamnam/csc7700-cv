...
\frame{ \frametitle{} 
 \includegraphics[width=\textwidth]{imgs/cam_calibration.jpg}
}

\frame{ \frametitle{} 
 \includegraphics[width=\textwidth]{imgs/obj_recognition.jpg}
}
...
\frame{ \frametitle{} 
 \includegraphics[width=\textwidth]{imgs/detect_describe_match.png}
}
...
\frame{ \frametitle{} 
 \begin{center}
 \includegraphics[width=.8\textwidth]{imgs/ex_geo_photo_diffs-fw.png}
 \end{center}
}
...
\frame{ \frametitle{Background}
 \begin{itemize}
 \item \textbf{Detection} of interest points is done through approximations of the \hyperlink{math-laplacian}{\beamergotobutton{Laplacian}} of \hyperlink{math-gaussian}{\beamergotobutton{Gaussians}}, then finding extrema within the scale space of the image. 
 \item \textbf{Description} is handled by assigning orientation vectors using \hyperlink{math-haar}{\beamergotobutton{Haar wavelets}} over a 4x4 grid. Four values ($d_x, d_y, |d_x|, |d_y|$) are stored for each cell, yielding a 64-dimension description vector.
 \item \textbf{Matching} is facilitated by indexing the results with the sign of the Laplacian, which indicates if the blob is block-on-white or white-on-black. The nearest-neighbor ratio matching is used.
 \end{itemize} 
}
...
and downsampling as in SIFT, but instead by \hyperlink{math-convolution}{\beamergotobutton{convolving}} with the image box filters (of increasing size) which approximate the \hyperlink{math-hessian}{\beamergotobutton{Laplacian}} of Gaussians.  Extrema of the Hessian determinants found within the \textbf{octaves} constituting the scale space of the image indicate blob responses.
\end{itemize} 
}
...
\begin{frame}
 \begin{center}
 \includegraphics[width=.6\textwidth]{imgs/haar-wavelet.png}
 \end{center}
\end{frame}
...
\frame{ \frametitle{}
 \begin{center}
 \includegraphics[width=.9\textwidth]{imgs/descriptor-vector.jpg}
 \end{center}
}
...
\end{equation}
  ...
  \item Note that:
\begin{equation}
  det(\mathcal{H}) = L_{xx} L_{yy} - (wL_{xy})^2.
\end{equation}
  \end{itemize}
}

\frame{ \frametitle{}
  \includegraphics[width=\textwidth]{imgs/box_filters.png}
}
...
\frame{ \frametitle{Hessian Determinant Approximation}
 ...
 \end{equation}
 where $|x|_F$ is the
 \hyperlink{math-frobenius}{\beamergotobutton{Frobenius norm}}. This factor changes slightly with filter size ($n$ is approximately $\frac{9}{1.2} \sigma$); however, it is desirable to keep it constant. Therefore the filter responses ($D$) are normalized with respect to their size to guarantee a constant Frobenius norm. 
...
\frame{\frametitle{} \includegraphics[width=\textwidth]{imgs/hessian_matrix.png}}
\frame{\frametitle{} \includegraphics[width=\textwidth]{imgs/hessian_approx_matrix.png}}
\frame{\frametitle{} \includegraphics[width=\textwidth]{imgs/det_hessian_approx_matrix.png}}
...
upscaled and computed, for which there is little additional computational cost. As a side effect of not downsampling the image, there is no \textbf{aliasing}. 
...
\begin{itemize}
  \item Standard testing data provided by Mikolajczyk, used to test detector and descriptor.
  \item Includes images of real textures and structured scenes, and different types of geometric and photometric transformations (varying viewpoint, zoom, rotation, lighting, blur...).
  \item To test applying this to 3D reconstruction, two views of the same object are evaluated to measure the angle between two planes.
  \item For n-views 3D reconstruction, 13 input images of an old vase are used.
  \item For the application to object recognition, data is taken from the Caltech face, background, airplane, and motorcycle databases.
 \end{itemize}
}

\frame{\frametitle{Results}\begin{center}\includegraphics[width=.8\textwidth]{imgs/results7.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.6\textwidth]{imgs/results8.jpg}\end{center}}

\subsection{Experimental Setup}
\frame{ \frametitle{Experimental Setup}
 \begin{itemize}
  \item All timings measured on a 3Ghz Pentium 4 machine.
  \item To test SURF, the authors simply use the testing software provided by Mikolajczyk.
  \item To test application to 3D reconstruction, SURF was added to the Epoch 3D Webservice of the VISICS research group at the K.U. Leuven
  \item To test application to image recognition, the experiment is based on the two bag-of-words classifer. The task is to determine if an object occurs in an image or not.
 \end{itemize}
}
...
\frame{\frametitle{Results}\begin{center}\includegraphics[width=\textwidth]{imgs/table1.png}\end{center}}

\frame{\frametitle{Results}\begin{center}\includegraphics[width=.8\textwidth]{imgs/results0a.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.8\textwidth]{imgs/results0b.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.8\textwidth]{imgs/results1a.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.8\textwidth]{imgs/results1b.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.8\textwidth]{imgs/results2a.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.8\textwidth]{imgs/results2b.jpg}\end{center}}
%\frame{\frametitle{Results}\begin{center}\includegraphics[width=.6\textwidth]{imgs/results3.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.6\textwidth]{imgs/results4.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.6\textwidth]{imgs/results5.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=\textwidth]{imgs/table2.png}\end{center}}
%\frame{\frametitle{Results}\begin{center}\includegraphics[width=.6\textwidth]{imgs/results6.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.6\textwidth]{imgs/results7.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.4\textwidth]{imgs/results8.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.8\textwidth]{imgs/results9a.jpg}\end{center}}
\frame{\frametitle{Results}\begin{center}\includegraphics[width=.8\textwidth]{imgs/results9b.jpg}\end{center}}
...
\begin{itemize} 
\item Speeded-Up Robust Features is an approach for detecting, describing, and matching interest points found in an image in a computationally fast but robust way.
\item The main metric of evaluation for detection is repeatability, which is the ability of an interest point to be detected under several varying circumstances.
\item For description it is uniqueness and completeness and for matching it is mainly speed and accuracy.
\end{itemize} 
}

\frame{ \frametitle{Conclusion}
\begin{itemize} 
\item Several approximations are used including box filters and haar wavelets along with several concepts such as integral images, gradient sums, and fast indexing in an attempt to achieve greater efficiency overall, but not at the cost of performance.
\item Careful tradeoffs were made and evaluated in an effort to pick the best combinations of parameters and to push approximations even further.
\item Through all these techniques, SURF became faster and more accurate than previous methods, including SIFT, Difference of Gaussians, and Hessian-Laplace.
\end{itemize} 
}
...
\begin{itemize} 
 \item H. Bay et al. SURF: speeded up robust features. Computer Vision and Image Understanding 346-359. Elsevier. 2008.
 \item http://www.wolframalpha.com/
 \item http://stuffyoudontwant.com/sports-equipment/soccer-balls/
 \item http://www.cs.ucf.edu/~mali/haar/
 \item http://en.wikipedia.org/wiki/Scale\_space
 \item http://www.comp.nus.edu.sg/~cs4243/lecture/imageproc.pdf
 \item http://rt.com/files/news/curiosity-rover-ready-drive-029/penny-camera-calibration-target.jpg
 \item http://www.cs.tau.ac.il/~wolf/ORWS/
 \end{itemize}  
}

\section{Mathematical Appendix}

\frame{ \frametitle{Index of Mathematical Concepts}
\begin{itemize} 
 \item \hyperlink{math-determinant}{\beamergotobutton{Determinant}}
 \item \hyperlink{math-convolution}{\beamergotobutton{Convolution}}
 \item \hyperlink{math-laplacian}{\beamergotobutton{Laplacian}}
 \item \hyperlink{math-gaussian}{\beamergotobutton{Gaussian}}
 \item \hyperlink{math-distance}{\beamergotobutton{Distance measures}}
 \item \hyperlink{math-integral}{\beamergotobutton{Integral images}}
 \item \hyperlink{math-frobenius}{\beamergotobutton{Frobenius norm}}
 \item \hyperlink{math-clairaut}{\beamergotobutton{Clairaut's theorem}}
 \item \hyperlink{math-hessian}{\beamergotobutton{Hessian matrix}}
 \item \hyperlink{math-haar}{\beamergotobutton{Haar wavelets}}
\end{itemize} 
}
